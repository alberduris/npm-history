...
export const GET: APIRoute = async ({ url }) => {
  ...
  if (!packagesParam) {
    return new Response('Missing "packages" query parameter', {
      status: 400,
  ...
  if (packageNames.length === 0) {
    return new Response('No valid package names provided', {
      status: 400,
      headers: { 'Cache-Control': 'no-store' },
    });
  }
  ...
  try {
    const { data: validResults, allComplete } = await fetchAndPreparePackages(packageNames);
    if (validResults.length === 0) {
      return new Response('No download data found for the specified packages', {
        status: 404,
        headers: { 'Cache-Control': 'no-store' },
      });
    }
